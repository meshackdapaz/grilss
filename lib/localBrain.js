...
🍟 *SIDES*: Peri Peri Fries (3k), Lemon Fries (4k), Rice (4k), Mash (4k).`;
    ...
    async getResponseFromHistory(prompt, history = []) {
        ...
        if (text.match(/^(habari|mambo|hello|hi|hey|niaje|vipi|mzima|shikamoo|uhali|ooh|oyaa|mzeiya|boss|bos|pamoja|kwema)/)) {
            return this.signature;
        }
        
        // 2. MENU / PRICES / "WHAT DO YOU SELL?" (Expanded Brilliance)
        if (text.match(/(menu|bei|price|how much|ngapi|orodha|mnauza|vyakula|chakula|msosi|misosi|kula|nini|mna nini|kuna nini|mnanini|vipo|maajabu|mlo|mlo gani|msosi gani|kuna misosi gani|nitakula nini|mautundu|vitu)/)) {
            return this.menu + "\n\nUnakula nini leo, Boss? (Andika jina la msosi, mfano: *Tomahawk* au *Wings*) 🥩";
        }

        // 3. ORDERING FOOD (Full Menu Logic - Expanded)
        if (text.match(/(steak|bone|ribeye|wings|nundu|poutine|chicken|strips|tomahawk|sirloin|platter|ribs|choma|kuku|chips|nataka|naitaji|nahitaji|nipatie|nipe|nichukulie|letae|leta|order|lamba|tuma)/)) {
            ...
            return `*ORDER YAKO IMEPOKELEWA* 📝\n\nSawa kabisa boss, tume-receive order ya *${item}*.\n\nKukamilisha order, tafadhali fanya malipo kwa Lipa Namba:\n${this.payment}`;
        }
        
        // 4. LOCATION (Expanded)
        if (text.match(/(wapi|location|mahali|place|mpo wapi|upanga|ramani|sehemu|njia|kuelekea|pande gani)/)) {
        ...
        return "Karibu sana Mjomba's Texas Grill! 🙏\n\nJe, ungependa nikutumie menu na bei zetu upate kuchagua msosi wako, boss? Tuko hapa kuhakikisha unapata huduma bora kabisa. 🥩🔥";
    }
}
